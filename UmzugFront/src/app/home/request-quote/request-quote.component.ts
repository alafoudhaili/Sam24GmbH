...
export class RequestQuoteComponent implements OnInit,AfterViewInit {
    ...
    if (this.addForm.invalid) {
      for (const i in this.addForm.controls) {
        if (this.addForm.controls[i].invalid) {
          switch (i) {
            case 'nom':
              this.sweetAlertService.danger('Name is mandatory !');
              break;
            case 'email':
              this.sweetAlertService.danger('Email is mandatory !');
              break;
            case 'telephone':
              this.sweetAlertService.danger('Phone is mandatory !');
              break;
            case 'message':
              this.sweetAlertService.danger('Message is mandatory !');
              break;
          }
          return;
        }
  ...
  sendMessage() {
    if (this.addForm.valid) {
      ...
      this.messageLandingService.addMessage(newMessage).subscribe((result) => {
            this.sweetAlertService.success('Message sent successfully');
            setTimeout(() => {
              this.router.navigate([''])
            }, 1000)
          }
          , error => {
            this.sweetAlertService.danger('Problem while sending message ! ');
          });
    }
  }
